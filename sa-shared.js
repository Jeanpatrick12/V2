    ...
    document.querySelectorAll("[data-nav-account]").forEach(wrap => {
      ...
      if (user) {
        ...
        dropdown.innerHTML =
          ...
          '<button class="sa-account-dropdown-logout" style="color:#333" onclick="window.top.location.href=\'mes-annonces\'"><i class="ti ti-home"></i> Mes annonces' + (getUserListings().length > 0 ? ' <span style="margin-left:6px;flex-shrink:0;display:inline-flex;align-items:center;justify-content:center;background:#E84533;color:white;font-size:9px;font-weight:700;width:16px;height:16px;aspect-ratio:1/1;border-radius:8px;box-sizing:border-box;padding:1px 4px 2px">' + getUserListings().length + '</span>' : '') + '</button>' +
          '<button class="sa-account-dropdown-logout" style="color:#333" onclick="window.top.location.href=\'documents\'"><i class="ti ti-files"></i> Mes documents' + (function(){ var n = getDocs().length; try { n += JSON.parse(localStorage.getItem("sa_buyer_docs_" + user.id) || "[]").length; } catch(e) {} return n > 0 ? ' <span style="margin-left:6px;flex-shrink:0;display:inline-flex;align-items:center;justify-content:center;background:#E84533;color:white;font-size:9px;font-weight:700;width:16px;height:16px;aspect-ratio:1/1;border-radius:8px;box-sizing:border-box;padding:1px 4px 2px">' + n + '</span>' : ''; })() + '</button>' +
